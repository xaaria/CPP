...
namespace otecpp_otevector {
  ...
  template<typename T> class OteVector {
    ...
    public:
      ...
      /**
       *  throw(std::out_of_range) jos indeksi menee yli rajojen 
       */ 
      T& operator[](int i) 
  ...
  */
  ...
  template<typename T> T& OteVector<T>::operator[](int i) throw(std::out_of_range) {
  ...
  template<typename T> const T& OteVector<T>::operator[](int i) const throw(std::out_of_range) {
    if(i < 0 || i >= koko) { 
